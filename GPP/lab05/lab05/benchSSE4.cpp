...
FORCEINLINE void executeVectorsSSE4(Vector3 *vectorList, Vector3 *vectorList2, int start, int finish)
{
...
int perftest_SSE4()
{
	...
	for(int j=0; j<NUMPASSES; j++)
	{
		...
		for(int t = 0; t < numThreads; t++)
		{
			...
			if(t == numThreads - 1)
				finish--;
			threads.create_thread(boost::bind(executeVectorsSSE4, vectorList, vectorList2, (int)(t * num), finish));
		}
		threads.join_all();
#else
